...
**Claude Desktop** — в `claude_desktop_config.json` (macOS: `~/Library/Application Support/Claude/`, Windows: `%APPDATA%\Claude\`):
...
Командой или вручную в `~/.codex/config.toml`:
...
В `.vscode/mcp.json` — обратите внимание: ключ `servers` (а не `mcpServers`) и поле `type`:
